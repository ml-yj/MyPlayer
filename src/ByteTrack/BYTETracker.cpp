...
vector<STrack*> BYTETracker::joint_stracks(vector<STrack*> &tlista, vector<STrack> &tlistb)
{
	...
	for (int i = 0; i < tlistb.size(); i++)
	{
		...
		if (!exists.count(tid) || exists.find(tid)->second != 1)
		{
			exists[tid] = 1;
...
void BYTETracker::linear_assignment(vector<vector<float> > &cost_matrix, int cost_matrix_size,
...
{
	vector<vector<float> > ious_result;
	if (atlbrs.size() * btlbrs.size() == 0)
		return ious_result;

	ious_result.resize(atlbrs.size());
	for (int i = 0; i < ious_result.size(); i++)
	{
		ious_result[i].resize(btlbrs.size());
	}

	// Compute IoU
	for (int k = 0; k < atlbrs.size(); k++)
	{
		for (int n = 0; n < btlbrs.size(); n++)
		{
			float box_area = (btlbrs[n][2] - btlbrs[n][0]) * (btlbrs[n][3] - btlbrs[n][1]);
			float iw = min(atlbrs[k][2], btlbrs[n][2]) - max(atlbrs[k][0], btlbrs[n][0]);
			if (iw > 0)
			{
				float ih = min(atlbrs[k][3], btlbrs[n][3]) - max(atlbrs[k][1], btlbrs[n][1]);
				if (ih > 0)
				{
					float ua = (atlbrs[k][2] - atlbrs[k][0]) * (atlbrs[k][3] - atlbrs[k][1]) + box_area - iw * ih;
					ious_result[k][n] = iw * ih / ua;
				}
			}
		}
	}
	...
	int &dist_size, int &dist_size_size)
{
	...
	dist_size = (int)atlbrs.size();
	dist_size_size = (int)btlbrs.size();
